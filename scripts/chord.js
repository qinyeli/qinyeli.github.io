...
var chords = (function(){
    ...
    var ChordBoxImage = function(name, chord, fingers, notes, size, dontDrawNut) {
        ...
        var InitializeSizes = function() {
            ...
            _noteNameFontSize = _fretWidth * 0.8
            ...
            _xstart = _fretWidth;
            _ystart = Math.round(0.2 * _superScriptFontSize + _nameFontSize
                                 + _nutHeight + 1.7 * _markerWidth);
        ...
        var ParseNotes = function(notes) {
            notes = String(notes)+'------';
            notes = notes.replace(/[^\-b#1234567]/g,'')
                    .replace(/b/g, String.fromCharCode(9837))
                    .replace(/#/g, String.fromCharCode(9839));

            for (var i = 0; i < notes.length && _notes.length < 6; i++) {
                note = notes.charAt(i)
                if (note >= '0' && note <= '7' || note === '-') {
                    _notes.push(note)
                } else {
                    if (i == notes.length - 1) break;
                    nextNote = notes.charAt(++i)
                    if (nextNote >= '0' && nextNote <= '7') {
                        _notes.push(note + nextNote)
                    }
                }
            }
        }
        ...
        var DrawChordBox = function() {
            ...
            if (_baseFret == 1 && !_dontDrawNut) {
                //Need to draw the nut
                var nutHeight = _fretWidth / 2;
                _graphics.FillRectangle(_foregroundBrush, _xstart - _lineWidth / 2,
                                        _ystart - nutHeight, _boxWidth, nutHeight);
            }
        };
        ...
        var DrawChordPositions = function() {
            ...
            for (var i = 0; i < _chordPositions.length; i++) {
                ...
                if (relativePos > 0) {
                    ...
                    DrawFinger(i, xpos + _dotWidth / 2, ypos)
        ...
        var DrawFinger = function(n, xpos, ypos) {
            var font = Font(FONT_NAME, _fingerFontSize);
            var finger = _fingers[n];
            if (finger != NO_FINGER) {
                 var charSize = _graphics.MeasureString(finger.toString(), font);
                _graphics.DrawString(finger.toString(), font, _backgroundBrush, xpos - (0.5 * charSize.Width), ypos);
            }

        }

        var DrawNoteNames = function() {
            var xpos = _xstart + (0.5 * _lineWidth);
            var ypos = _ystart + _boxHeight;
            var font = Font(FONT_NAME, _noteNameFontSize);
            for (var n=0; n<_notes.length; n++) {
                var note = _notes[n];
                if (note != NO_NOTE_NAME) {
                    var charSize = _graphics.MeasureString(note.toString(), font)
                    _graphics.DrawString(note.toString(), font, _foregroundBrush, xpos - (0.5 * charSize.Width), ypos);
                }
        ...
        var DrawChordName = function() {
            ...
            var superIndex = _chordName.indexOf("^")
            var subIndex = _chordName.indexOf("_")
            if (superIndex == -1 && subIndex == -1) {
                name = _chordName;
            } else if (superIndex == -1) {
                [name, subscription] = _chordName.split('_');
            } else if (subIndex == -1) {
                [name, superscription] = _chordName.split('^');
            } else if (subIndex < superIndex) {
                [name, temp] = _chordName.split('_');
                [subscription, superscription] = temp.split('^');
            } else {
                [name, temp] = _chordName.split('^');
                [superscription, subscription] = temp.split('_');
            }
            ...
            _graphics.DrawString(name, nameFont, _foregroundBrush,
                                 xTextStart, 0);
            if (superscription != "") {
                _graphics.DrawString(superscription, superFont, _foregroundBrush,
                                     xTextStart + stringSize.Width,
                                     - 0.2 * _superScriptFontSize);
            }
            if (subscription != "") {
                _graphics.DrawString(subscription, superFont, _foregroundBrush,
                                     xTextStart + stringSize.Width,
                                     _nameFontSize - 0.8 * _superScriptFontSize);
            }

            if (_baseFret > 1) {
                var fretFont = Font(FONT_NAME, _fretFontSize);
                var offset = (_fretFontSize - _fretWidth) / 2;
                _graphics.DrawString(_baseFret + "fr", fretFont, _foregroundBrush,
                                     _xstart + _boxWidth + 0.4 * _fretWidth, _ystart - offset);
            }
        }
    ...
    var ReplaceChordElements = function() {
          ...
          for(var i=0; i<chords.length; ++i) {
            ...
            var notes = elt.getAttribute('notes')
            var size = elt.getAttribute('size');
            var dontDrawNut = elt.getAttribute('dontDrawNut');

            var chord = ChordBoxImage(name, positions, fingers, notes, size, dontDrawNut);
